...
cam = GradientShap(net)
baselines = torch.randn(10, 3, 224, 224).to(device)
...
def get_auc_per_data_subset(range_index, net, cam, num=num_sample):
    ...
    for j, (img, trg) in enumerate(tqdm(data_loader, total=len(data_loader), desc='Loading images')):
        ...
        grayscale_gradcam = cam.attribute(img, baselines, target=trg)
        grayscale_gradcam = np.sum(grayscale_gradcam.squeeze().cpu().detach().numpy(), axis=1)
        grayscale_gradcam = attr_zero_norm(grayscale_gradcam)
        # grayscale_gradcam = np.abs(np.sum(grayscale_gradcam.cpu().detach().numpy(), axis=1))
