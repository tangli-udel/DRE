...
class CausalMetric():
    ...
    def evaluate(self, img_batch, exp_batch, batch_size):
        ...
        if n_samples % batch_size != 0:
            start_index = n_samples // batch_size
            preds = self.model(img_batch[start_index*batch_size:].cuda()).cpu()
            predictions[start_index*batch_size:] = preds
        ...
        if n_samples % batch_size != 0:
            start_index = n_samples // batch_size
            substrate[start_index*batch_size:] = self.substrate_fn(img_batch[start_index*batch_size:])
        ...
        for i in tqdm(range(n_steps+1), desc=caption + 'pixels'):
            ...
            if n_samples % batch_size != 0:
                start_index = n_samples // batch_size
                preds = self.model(start[start_index*batch_size:].cuda())
                preds = preds.cpu().numpy()[range(n_samples%batch_size), top[start_index*batch_size:]]
                scores[i, start_index*batch_size:] = preds
